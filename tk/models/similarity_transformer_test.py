...
class TestSimilarityTransformerDevModel(tf.test.TestCase):
  ...
  def test_infer(self):
    ...
    _ = model.infer(encode(query, encoders=self.encoders))
  ...
  def test_pair_distance(self):
    ...
    code1_emb = model.infer(encode(code1, encoders=self.encoders))
    code2_emb = model.infer(encode(code2, encoders=self.encoders))

    model.loss(code1_emb, code2_emb, "kfnet")
